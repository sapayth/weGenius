...
class ApiController {
	...
	public function get_dashboard_overview( $request ) {
		...
		$analysis_types = $this->get_analysis_types_data( $filters );
	...
	private function submit_to_external_api( $data ) {
		...
		// Make API request
		$response = wp_remote_post( $api_endpoint . '/articles/submit', [
			'headers' => [
	...
	private function schedule_analysis( $analysis_id, $post_id, $analysis_types ) {
		...
		if ( ! function_exists( 'as_schedule_single_action' ) ) {
			...
			error_log( 'WeGenius: Action Scheduler is required but not available. Analysis failed for post ' . $post_id );
			return;
		}
	...
	private function sanitize_settings( $settings ) {
		...
		if ( isset( $settings['analysis'] ) ) {
			$sanitized['analysis'] = [
				'defaultAnalysisTypes' => array_map( 'sanitize_text_field', $settings['analysis']['defaultAnalysisTypes'] ?? [] ),
				'autoAnalyze' => (bool) ( $settings['analysis']['autoAnalyze'] ?? false ),
				'reanalysisFrequency' => sanitize_text_field( $settings['analysis']['reanalysisFrequency'] ?? 'never' ),
				'minContentLength' => absint( $settings['analysis']['minContentLength'] ?? 100 ),
				'contentTypes' => array_map( 'sanitize_text_field', $settings['analysis']['contentTypes'] ?? [] ),
		...
		if ( isset( $settings['contentGeneration'] ) ) {
			$sanitized['contentGeneration'] = [
				'writingTones' => [
					'primary' => sanitize_text_field( $settings['contentGeneration']['writingTones']['primary'] ?? 'professional' ),
					'secondary' => sanitize_text_field( $settings['contentGeneration']['writingTones']['secondary'] ?? 'authoritative' ),
				],
				'targetWordCount' => absint( $settings['contentGeneration']['targetWordCount'] ?? 1000 ),
				'targetAudience' => sanitize_text_field( $settings['contentGeneration']['targetAudience'] ?? 'Parents' ),
				'includeFeaturedImage' => (bool) ( $settings['contentGeneration']['includeFeaturedImage'] ?? false ),
				'location' => sanitize_text_field( $settings['contentGeneration']['location'] ?? 'Texas, USA' ),
				...
				'videoStyle' => sanitize_text_field( $settings['contentGeneration']['videoStyle'] ?? 'professional' ),
				...
				'voiceGender' => sanitize_text_field( $settings['contentGeneration']['voiceGender'] ?? 'male' ),
				'addBackgroundMusic' => (bool) ( $settings['contentGeneration']['addBackgroundMusic'] ?? false ),
				'musicMood' => sanitize_text_field( $settings['contentGeneration']['musicMood'] ?? 'calm' ),
			];
		}
	...
	public function get_analysis_status_for_post( $request ) {
		...
		if ( $job_id && function_exists( 'as_get_scheduled_action' ) ) {
			...
			if ( $scheduled_action ) {
				...
				$response['job_scheduled_for'] = date( 'c', $scheduled_action->get_schedule()->get_date()->getTimestamp() );
			} else {
